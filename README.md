...
以下截图来自本地运行的 `http://localhost:5173`，分别对应项目的核心功能：

| 页面 | 建议文件名 | 展示重点 |
| --- | --- | --- |
| 梗图列表页 | `docs/screenshots/meme-library.png` | 搜索、卡片网格和上传入口 |
| 上传抽屉 | `docs/screenshots/upload-drawer.png` | `Ctrl+V` 粘贴、预览和上传进度 |
| 梗图详情 | `docs/screenshots/meme-detail.png` | AI 标题、描述、标签和 OCR 结果 |
| 编辑梗图 | `docs/screenshots/meme-edit.png` | 标题、描述和标签编辑 |
| 搜索结果 | `docs/screenshots/meme-search.png` | URL 搜索参数和筛选结果 |
| AI 设置页 | `docs/screenshots/ai-settings.png` | 分析 AI / 内容 AI 配置拆分 |

### 页面截图

#### 梗图列表

![梗图列表页](docs/screenshots/meme-library.png)

#### 梗图详情

![梗图详情弹窗](docs/screenshots/meme-detail.png)

#### 编辑梗图

![编辑梗图弹窗](docs/screenshots/meme-edit.png)

#### 上传抽屉

![上传梗图抽屉](docs/screenshots/upload-drawer.png)

#### 搜索结果

![梗图搜索结果](docs/screenshots/meme-search.png)

#### AI 设置

![AI 设置页](docs/screenshots/ai-settings.png)

截图目录的保存规范见 [`docs/screenshots/README.md`](docs/screenshots/README.md)。截图中的 API Key 已做掩码处理。

## 求职展示重点

这个项目适合重点展示以下工程能力：

- 从前端交互到后端存储、数据库和 AI 服务的完整闭环。
- 使用 `multipart/form-data` 上传图片，并通过本地静态资源访问。
- 使用 Prisma、DTO 和全局校验保证后端数据边界清晰。
- 使用 TanStack Query 管理真实 API 数据和列表刷新。
- 将图片分析 AI 与未来的内容生成 AI 分开配置，保留后续扩展空间。
- 对上传失败、AI 失败、重新分析和剪贴板粘贴等边界场景提供处理。

完整的 1～2 分钟演示脚本见 [`docs/demo-flow.md`](docs/demo-flow.md)。
...
```mermaid
flowchart LR
  UI[React 前端] --> Query[TanStack Query]
  Query --> Client[统一 API Client]
  Client --> API[NestJS API]
  API --> Meme[Meme Service]
  Meme --> Prisma[Prisma]
  Prisma --> DB[(PostgreSQL)]
  Meme --> Storage[本地图片存储]
  Meme --> AI[AI Service]
  AI --> Vision[OpenAI 兼容视觉模型]
```
