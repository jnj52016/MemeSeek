...
- 当前统一使用 OpenAI API，分析 AI 使用支持图片输入的视觉模型。
- 默认提示词固定在后端，不在前端编辑。
- 梗图图片识别使用 OpenAI 视觉模型，图片通过 `image_url` 传入。
...
- [x] 确认图片输入方案：使用 OpenAI 视觉模型。
- [x] 校验 AI 返回的 JSON。
- [x] 保存标题、描述、标签和 OCR 文字。
- [x] 处理 AI 分析失败。
- [x] 实现重新分析接口。
- [x] 配置 OpenAI 视觉请求和默认模型。
- [x] 使用真实 OpenAI API Key 完成端到端图片分析。
