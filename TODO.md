...
项目下一阶段不再优先把用户媒体永久保存到后端，而是改造成“本地优先”的梗图管理网页：

- 用户首次使用时选择电脑上的梗图文件夹，并授予网页读写权限。
- 媒体文件、封面和素材库索引保存在用户选择的本地目录。
- IndexedDB 保存目录句柄，用于恢复访问权限。
- 不做注册、登录、CloudBase 云存储和跨设备同步。
- CloudBase 主要托管前端；后端仅保留不持久化文件的 AI 临时分析接口。

设计和实施以以下文档为准：

- [本地文件夹存储改造 TODO](docs/local-first-storage-todo.md)
- [本地模式 OpenAPI 3.1 契约](docs/openapi-local-first.yaml)

当前下一步：执行本地文件夹改造 P0，实现浏览器能力检测、目录选择、IndexedDB 句柄保存和权限恢复。
...
视频上传、首帧生成、视频校验、静态访问、删除清理、AI 首帧传递和前端展示测试已补充
        ↓
下一步：转入本地文件夹模式 P0，并在改造过程中持续执行回归测试
```

说明：以下内容记录改造前的 PostgreSQL + 后端本地上传版本，作为功能迁移和回归测试基线；新的需求状态以本地文件夹存储改造 TODO 为准。
