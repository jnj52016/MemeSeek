...
真实 OpenAI 视觉模型端到端验证已完成
        ↓
视频媒体数据模型、磁盘上传、视频预览、封面处理和关键帧分析代码已接入
        ↓
...
当前项目的上传、存储、预览和 AI 分析链路支持图片、视频以及 GIF/WebP 动图。本阶段不直接把完整视频或长动图发送给当前的 OpenAI 兼容 Chat Completions 接口。
...
保存原视频并读取媒体元数据
    ↓
FFmpeg 处理视频和动图：短动图抽取第一帧，长动图或视频抽取关键帧
    ↓
视觉 AI 分析关键帧、时间点和画面语境
    ↓
保存标题、描述、标签和 OCR
    ↓
前端展示视频播放器和 AI 分析结果
...
- [x] 增加视频和 GIF/WebP 动图封面、关键帧及临时文件的生命周期管理。
- [x] 删除媒体时同时删除原视频、封面、关键帧和其他派生文件。

### FFmpeg 媒体处理

- [x] 接入 `ffmpeg` 和 `ffprobe` 配置；当前环境尚未检测到这两个命令，支持通过环境变量指定路径。
- [x] 使用 `ffprobe` 读取视频时长。
- [x] 按时间均匀抽取最多 8 张关键帧，并记录每张关键帧的时间点。
- [x] GIF/WebP 动图不超过 3 秒时只抽取第一帧，超过 3 秒时抽取最多 8 张关键帧。
- [x] 生成适合列表卡片和详情页使用的封面图。
- [x] 设计 FFmpeg 失败时的错误提示、临时文件清理和数据库回滚逻辑。

### AI 视频与动图分析

- [x] 复用 `POST /memes/:id/analyze`，根据 `mediaType` 和 GIF/WebP MIME 类型处理图片、视频和动图。
- [x] 将视频或长动图关键帧压缩后作为多个 `image_url` 输入发送给现有视觉模型；短动图只发送第一帧。
- [x] 更新分析提示词，使模型理解视频的时间顺序、画面变化和使用语境。
...
- [x] 修改 `MemeCard`，使用视频封面展示视频素材，不默认自动播放原视频。
- [x] 动图列表卡片使用上传时生成的 JPEG 封面，详情弹窗仍展示原始动图。
